...
if (!empty($email) || !empty($username) || !empty($password) || !empty($phone) || !empty($city) || !empty($height) || !empty($weight) || !empty($address) || !empty($country)) {
 $host = "localhost";
    ...
    if (mysqli_connect_error()) {
     ...
     $SELECT = "SELECT email From policee Where email = ? Limit 1";
     $INSERT = "INSERT Into policee (email, username, password, phone, city, height, weight, address, country) values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
     //Prepare statement
     ...
     if ($rnum==0) {
      ...
      $stmt->execute();
      echo "DETAILS OF THE POLICE HAS BEEN ADDED";
     } else {
